...
from game.shadows.shadow import Shadow
...
class ShadowCaster:
    def __init__(self, map):
        ...
        self.render_corners = [(0, 0), (self.render_width, 0), (self.render_width, self.render_height),
                               (0, self.render_height)]
    ...
    def generate_shadows(self, source):
        ...
        for i, obstacle in enumerate(sorted(self.map.obstacles, key=lambda x: x.distance)):
            ...
            for corner in obstacle.corners:
                ...
                if vector_length == 0: continue  # Evitam impartirea la 0
            ...
            if len(allpoints) < 3: continue

            shadow_polygon = geometry.MultiPoint(allpoints).convex_hull

            # Verificare extra pentru validitate
            if not isinstance(shadow_polygon, geometry.Polygon):
                continue
        ...
        if isinstance(render_shapes, geometry.GeometryCollection):
            for geom in render_shapes.geoms:
                if isinstance(geom, geometry.Polygon):
                    ...
                    self.shaded_areas.append(Shadow(geom, corners))
    ...
    def render_debug(self):
        self.debug_render_surface.fill((0, 0, 0, 0))
